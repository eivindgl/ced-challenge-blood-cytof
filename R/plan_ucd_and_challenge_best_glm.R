...
ucd_save_glm_sub_coeffs <- function(ucd_chal_glm_res_mixpanel, ucd_chal_glm_res_newpanel, 
                                ucd_chal_glm_res_oldpanel, ucd_chal_glm_scores) {
  dir_create('out')
  extract_coeffs(ucd_chal_glm_res_mixpanel) %>% 
    writexl::write_xlsx('out/ucd_and_chal_mixpanel_top_N_predictors_markers.xlsx') 
  extract_coeffs(ucd_chal_glm_res_oldpanel) %>% 
    writexl::write_xlsx('out/ucd_and_chal_oldpanel_top_N_predictors_markers.xlsx') 
  extract_coeffs(ucd_chal_glm_res_newpanel) %>% 
    writexl::write_xlsx('out/ucd_and_chal_newpanel_top_N_predictors_markers.xlsx') 
  
  ucd_chal_glm_scores %>% 
    writexl::write_xlsx('out/ucd_chal_glm_scores.xlsx')
  
  p <- roc_plot(ucd_chal_glm_scores)
  ggsave('out/ucd_and_chal_roc_auc_markers.png', plot = p)
}
